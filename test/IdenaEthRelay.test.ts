...
contract('IdenaEthRelay', (accounts) => {
    ...
    describe('...when used with 10000 adresses', () => {
        ...
        it('...should relay identities with very little gas', async () => {
            ...
            const bigIdentities0 = [...Array(10000).keys()]
            const bigLeaves0 = [...Array(10000).keys()]
            ...
            const bigState0 = bigTree0.getHexRoot() + bigIdentities0.map(bigIdentity => bigAccounts[bigIdentity].slice(2)).join("");

            IdenaEthRelayInstance = await IdenaEthRelay.new(bigState0, { gas: 0xfffffffffff, gasPrice: 2 });

            // check new identities
            for (let i in bigIdentities0) {
                ((await IdenaEthRelayInstance.isIdentity("0", bigAccounts[bigIdentities0[i]]))).should.be.equal(true);
            }
        });
